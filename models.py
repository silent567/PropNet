...
class IterPropModule(nn.Module):
    def __init__(self, args, input_dim, output_dim, batch=True, residual=False, use_gpu=False):

        super(IterPropModule, self).__init__()
        ...
        self.confidence_module = nn.Sequential(
            nn.Linear(nf_effect, int(nf_effect/2)),
            nn.LeakyReLU(),
            nn.Linear(int(nf_effect/2), 1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, state, Rr, Rs, Ra, pstep, verbose=0):
        ...
        left_confidence = torch.ones_like(state[:,0:1,0:1])
        out_particle_effect = particle_effect
        for i in range(pstep):
            if torch.max(left_confidence).item() > 1e-7:
                ...
                # calculate confidence / iterative related
                global_feat = torch.max(particle_effect, dim=1)[0]
                current_confidence = self.confidence_module(global_feat).unsqueeze(1)
                out_particle_effect = out_particle_effect + \
                    left_confidence*current_confidence*particle_effect
                left_confidence = left_confidence * (1.-current_confidence)
            else:
                break;
        particle_effect = out_particle_effect
...
class IterPropNet(nn.Module):
    def __init__(self, args, residual=False, use_gpu=False):
        super(IterPropNet, self).__init__()
        ...
        if args.pn_mode == 'partial':
            batch = False
            input_dim = attr_dim + state_dim
            self.encoder = IterPropModule(args, input_dim, nf_effect, batch, residual, use_gpu)
            self.decoder = IterPropModule(args, nf_effect, state_dim, batch, residual, use_gpu)
            ...
            batch = True
            input_dim = attr_dim + state_dim + action_dim
            self.model = IterPropModule(args, input_dim, position_dim, batch, residual, use_gpu)
